...
salaryList = [i * 1000 for i in range(10, 30, 2)]
job_dict = {}


def createTel():
    first = [133, 153, 180, 181, 189, 173, 177, 149, 130, 131, 132, 155, 156,
             185, 186, 145, 176, 185, 134, 135, 136, 137, 138, 139, 150, 151,
             152, 158, 159, 182, 183, 184, 147, 178]
    second = random.randint(0, 99999999)
    return f"{random.choice(first)}{second:8d}"


def createDepId(job_id):
    if job_id <= 9:
        return 1
    elif job_id <= 15:
        return 2
...
class employee:
    def __init__(self):
        self.name, self.sex = random_name()
        self.passwd = 123456
        self.sex_str = '男' if self.sex == 1 else '女'
        self.cardId = IdNumber.generate_id(self.sex)
        self.age = IdNumber(self.cardId).get_age()
        self.salary = random.choice(salaryList)
        self.birth = IdNumber(self.cardId).get_birthday()
        self.address = IdNumber(self.cardId).get_area_name()
        self.tel = createTel()
        self.job_id = random.randint(1, 39)
        self.dep_id = createDepId(self.job_id)
...
if __name__ == '__main__':
    ...
    for i in range(200):
        e = employee()
        sql = f"insert into emp_ms.employee values (" \
              f"{i + 3}, '{e.name}', '{e.passwd}', '{e.sex_str}', {e.age}, {e.salary}, " \
              f"'{e.birth}' ,'{e.address}', '{e.cardId}', '{e.address}',' {e.tel}', {e.dep_id}, {e.job_id})"
        print(sql)
        cursor.execute(sql)
    ...
    connect.commit()
    print("新建职工表成功")
    data = cursor.execute("select * from emp_ms.employee")
...
# def leoRandName():
#     lastName = ["王", "李", "张", "赵", "孙", "马", "郭", "高", "宋", "梁", "何", "冯", "邓", "陈", "黄", "周", "吴", "徐", "朱", "胡", "林", "何", "罗", "郑"]
#     firstName = ["英", "华", "玉", "秀", "文", "明", "兰", "金", "国", "春", "红", "丽", "小", "梅", "云", "芳", "平", "海", "珍", "荣"]
#     for i in range(10):
#         print(random.choice(lastName) + random.choice(firstName))
#         print(random.choice(lastName) + random.choice(firstName) + random.choice(firstName))
#
#
