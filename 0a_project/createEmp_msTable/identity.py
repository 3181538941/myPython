...
if __name__ == '__main__':
    ...
    cardId = IdNumber.generate_id(random_sex)
    print(cardId)  # 随机生成身份证号
    print(IdNumber(cardId).area_id)  # 地址编码:410326
    print(IdNumber(cardId).get_area_name())  # 地址:河南省洛阳市汝阳县
    print(IdNumber(cardId).get_birthday())  # 生日:1995-7-10
    print(IdNumber(cardId).get_age())  # 年龄:23(岁)
    print(IdNumber(cardId).get_sex())  # 性别:1(男)
    print(IdNumber(cardId).get_check_digit())  # 校验码:7
    print(IdNumber.verify_id(cardId))  # 检验身份证是否正确:False
